export default function BrowseItemCard({ item, onClick }) {
  const displayPrice = item.price === 0 ? "Free" : `₹${item.price}`;
  // For the mockup, we want a "Negotiable" tag next to the price
  const isNegotiable = true; // Hardcoded for mockup
  ...
  return (
    <div 
      ...
      className="group bg-surface rounded-2xl shadow-sm border border-border overflow-hidden hover:shadow-lg hover:border-indigo hover:-translate-y-1 transition-all duration-300 flex flex-col h-full cursor-pointer"
    >
